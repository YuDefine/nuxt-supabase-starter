...
| 類型      | 數量     | 說明                                      |
| --------- | -------- | ----------------------------------------- |
| Commands  | 15–16 個 | 4 共用 + 11–12 SDD（依 OpenSpec/Spectra） |
| SubAgents | 3 個     | 自動執行特定任務的專家                    |
| Skills    | 39–40 個 | 23 通用 + 5 情境 + 11–12 SDD              |
...
使用 `pnpm sdd:select` 選擇 OpenSpec 或 Spectra。
...
詳見 [OPENSPEC.md](./OPENSPEC.md) 或 Spectra 文件。
...
OpenSpec 路線：11 個 skills（`openspec-*`）
Spectra 路線：12 個 skills（`spectra-*`）
