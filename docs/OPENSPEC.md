...
OpenSpec 是一套輕量級的「規格驅動開發」框架，專為現有專案設計。它把功能開發拆成三個階段：
...
實作完成後，用 `/opsx:verify` 確認符合規格：

```
/opsx:verify add-todos
```
...
可以使用 `/opsx:ff` 快速生成所有 artifacts：

```
/opsx:ff 新增待辦事項功能
```

但對於複雜功能，建議逐步進行以確保每個階段都正確。
...
- [OpenSpec 官方文件](https://openspec.dev/)
