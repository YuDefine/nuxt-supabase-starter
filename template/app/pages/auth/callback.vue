<script setup lang="ts">
  ...
  const { fetchSession, loggedIn } = useUserSession()
  const { message: errorMessage, hasError, setError } = useAuthError()
  const loading = ref(true)

  onMounted(async () => {
    try {
      // After OAuth redirect, fetch the session to complete authentication
      await fetchSession({ force: true })

      if (loggedIn.value) {
        // Success — redirect to home or the intended page
