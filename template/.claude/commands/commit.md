...
```bash
PRECHECK_JSON=$(node .claude/scripts/commit-precheck.mjs)
PRECHECK_EXIT=$?
...
- **MUST** 把 LOCKED 檔（`.claude/rules/` / `.claude/commands/` / `.claude/skills/spectra-*/` / `.claude/agents/` / `scripts/spectra-advanced/`）獨立成 `🧹 chore(clade)` group，跟業務 commit 分開
