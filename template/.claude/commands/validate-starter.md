...
- [ ] `commands/` 存在且包含根層命令檔
- [ ] `agents/` 存在
- [ ] `agents/code-review.md` 存在
- [ ] `agents/references/project-review-rules.md` 存在
- [ ] `hooks/` 存在
- [ ] `rules/` 存在
- [ ] `rules/truth-layers.md` 存在
- [ ] `skills/` 存在
- [ ] `settings.json` 存在
...
- [ ] `docs/decisions/` 存在且包含 starter 的分層真相基線
...
echo "=== 數量統計 ==="
ROOT_COUNT=$(ls .claude/commands/*.md | wc -l | tr -d ' ')
echo "根層: $ROOT_COUNT"
echo "# 預期：10 個以上"
```

預期至少包含的根層命令：`commit.md`, `db-migration.md`, `doc-sync.md`, `freeze.md`, `guard.md`, `ship.md`, `unfreeze.md`, `validate-starter.md`
