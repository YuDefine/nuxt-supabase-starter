...
走 [void.cloud](https://void.cloud) VoidZero 部署平台的 consumer（建在 Cloudflare Workers 上，但 deploy pipeline 由 void 接管）。Fleet 採用：<consumer-j>、co-purchase、quotation-generator（migration 中）。
...
→ Fleet 為什麼沒事到 2026-05：所有 wrangler-action consumer 讀 `.output/server/wrangler.json`，flag 自動對齊；只有 void.cloud track 暴露 void.json 與 wrangler.json 的 flag 落差。<consumer-j>（2026-05-25）+ co-purchase（2026-05-27）兩例撞 10021 後 promote 成本 §。
...
**Fleet 採用**：<consumer-i>（co-purchase 2026-05-26 前採此 pattern，遷 void.cloud 後改走 § 4.5）
...
**Fleet 採用**：<consumer-j>、co-purchase、quotation-generator（migration 中）
...
| <consumer-i> | Cloudflare D1 | A (wrangler-action) | ✅ | n/a |
...
| **<consumer-j>** | Cloudflare D1（void 自管，via @nuxt/content adapter）| **B (void.cloud)** | ❌ | current void 0.10；配置 3 沿用已驗證 production baseline；workflow 已改 GitHub OIDC，需一次性 connect |
...
YuDefine fleet 多個 consumer（<consumer-i> / co-purchase / <consumer-j>）的 CI / deploy workflow 跑在自架 runner `runs-on: [self-hosted, gh-runner-lxc]`（YuDefine LXC，省 GitHub-hosted minutes）。Self-hosted runner 跟 GitHub-hosted runner 有兩個關鍵差異，沿用 GitHub-hosted 的 workflow 寫法會踩坑：
...
| <consumer-i> | self-hosted, gh-runner-lxc | ❌（仍 `cache: pnpm`） | ❌ | — |
| <consumer-j> | self-hosted, gh-runner-lxc | ✅ | ✅ | GitHub OIDC + VOID_PROJECT |

<consumer-j> 的 `.github/workflows/deploy.yml` 是 current void.cloud + GitHub OIDC reference；co-purchase / <consumer-i> 的遷移由該 consumer 的 session 執行，clade 稽核命中時 **MUST relay 給它**
