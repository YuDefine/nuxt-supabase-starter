...
| `package.heavy-gate.*` | 2026-09-06 CPMS：typecheck／test／build 有 script，卻沒有對應重型准入；與 `audit-gate-coverage.ts` 共用 label、轉呼與 arg-safe 判定 |
| `package.doctor.dependency`／`package.doctor.installed`／`package.doctor.adapter` | 同日 CPMS：doctor script 存在，但 vite-doctor binary 缺席；檢查依賴宣告、本地可執行檔與 script 引用的投影入口 |
