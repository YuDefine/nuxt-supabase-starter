...
Diagnostic-only（exit 0）；consumer 端落地 **MUST relay 給該 consumer 的 session**（per
[[clade-role-and-todo-discipline]] § Consumer 工作命中時 MUST relay；brief 只給 findings + pointer +
驗收，**NEVER** 替它拆步驟）。exit 0 說的是「這支不擋你」，**NEVER** 讀成「所以不必送出去」。
