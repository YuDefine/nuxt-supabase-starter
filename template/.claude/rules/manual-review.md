...
3. 對應 cookbook 路徑：
   ...
   - Reference impl: <consumer-a> `packages/core/server/routes/auth/_dev-login.get.ts` / <consumer-b> `server/routes/auth/__test-login.get.ts` / <consumer-d> `server/routes/auth/_dev-login.get.ts` / rental-scout `server/routes/auth/_dev-login.get.ts` (commit f9f8930)
