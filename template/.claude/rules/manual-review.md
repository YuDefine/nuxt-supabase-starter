...
- `[verify:api]` — 純 HTTP round-trip（curl / ofetch / fetch）。主線跑 request，通過後寫 `(verified-api: <ISO> <METHOD> <URL> <STATUS>[ body=<hash>])` annotation。**裝 `nuxt-csurf` 的 consumer**（perno / nuxt-edge-agentic-rag / yuntech-usr-sroi 等）走 `vendor/snippets/verify-channels/api-roundtrip.template.sh` 的 dual-token recipe（harvest `csrf=<uuid>` cookie + HTML `<meta name="csrf-token">` token，POST 帶 `-H "csrf-token: <token>"`）；不走 dual-token 第一次就會撞 403 `CSRF Cookie not found`。
