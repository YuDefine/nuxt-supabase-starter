...
- spectra-apply Step 8b handoff message **之前**，主線 **MUST** 對每個 evidence-missing item 跑一輪 self-collect（per spectra-apply SKILL.md Step 8a Baseline-exists-but-functional-gap 自接路徑 (a)(b)(c)(d)）
- 成功 → 寫 `(verified-*:)` annotation，review-gui auto-check helper 自動勾 `[x]`
- 失敗 → 寫 `（deferred: tried (a)(b)(c)(d), <reason>）` annotation，註明已嘗試 path
- 跑完一輪後**仍** evidence-missing 的 item 才進 review-gui handoff，user 在 GUI 看到的應該是「剩下真需要 user 拍板（真機 / 視覺主觀 / production 授權 / OAuth-only path 不可達）的」收斂集合

**Fallback flow**（user 主動觸發；以下情境才合用）：

- **跨 session 補拍**：spectra-apply 已 archive、user 在後續新 session 想補 visual / journey evidence
- **主線 self-collect fail 後 user 想 ad-hoc 拍**：user 想用瀏覽器手拍、條碼槍真機、kiosk 平板等 agent 不能達的工具
- **User 主動加 extra evidence**（非系統要求）

**NEVER** 把「補 evidence prompt」當 default 入口 — 那等於把 self-automatable evidence collection 變成 user 手動儀式（per [[pitfall-verify-evidence-handoff-instead-of-self-collect]]）。User 在 review-gui 看到「⚠ N item 缺 evidence」按鈕本來就是 fallback exit，**NEVER** 設計上預期 user 必須點才能由 Claude 接手 — Claude 在 Step 8b 就該已經跑過一輪。

判別測試：「user 打開 review-gui 看到 evidence missing 時，是否還需要點按鈕才能 Claude 接手？」是 → 規約缺。否 → 正確路徑（主線已盡力，剩 user 不能自動化的）。
