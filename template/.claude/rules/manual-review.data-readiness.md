...
`ready_signal` 來源：主線在 Verify Channel Pass dispatch verify:ui 時，從 item 描述的**具體可斷言短語**建 structured signal（`text` / `text_all` / `text_any` / `selector` / `regex` / `min_rows`）。因此 item 描述本身 **MUST** 含一個具體、唯一、會出現在畫面上的斷言點（例「建議刀位壽命 143 秒」「逾期 badge」「`data-testid=suggested-baseline-row-T990201` 這列」），**NEVER** 只寫「畫面正常」「顯示資料」「狀態正確」這類無法 poll 的模糊語。
...
- 不影響 review-gui 對 evidence trail 的檢查（`(verified-e2e: ...)` / `(claude-discussed: ...)` 仍 enforce；archive-gate 已於 2026-09-07 退役，這一層的執行者現在只剩 review-gui）
