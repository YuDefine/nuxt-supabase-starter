...
1. archive 階段主線 orchestrator **主動** Read tasks.md `## 人工檢查` 區塊，識別未勾 `[discuss]` items
2. 對每條 item，主線 orchestrator 分類 trigger condition：
   ...
   - **External signal pending** — required deploy / soak / authorization **尚未**發生；目前 runtime 無法用分析合成 evidence
3. 對非 pending 的兩類，主線 orchestrator 主動準備 evidence（grep 結果、diff、command output、data summary、合理性分析）— **不要**等使用者開口
...
| 狀態 | Surface |
| --- | --- |
| 商業／prod 授權，packet 已備妥（`(awaiting-user-decision:)`） | work-loop `awaiting[]` + runtime-supported input surface 或直接對話（attended）／Packaging SOP（unattended）。未收到回答前保持 pending。`\my` 看得到。 |
| archive 當下還未勾的 `[discuss]` | spectra-archive Step 2.5 walkthrough（本檔上方） |
| External signal pending | Defer-to-HANDOFF（本檔上方），不是 inbox |
...
2. **Discuss** — 能現在拍板的（packet 已備妥／internal evidence 齊）走 work-loop `awaiting[]`；其餘等 archive Step 2.5 walkthrough。**NEVER** 塞進 review-gui
3. **archive 階段 review GUI** — 只處理未勾 `[review:ui]`（以及 TD-658 範圍內、人要點 OK 的 `[verify:ui]`）。User 在 GUI 看 evidence/screenshot 點 OK / Issue / Skip
...
1. **User round 2 評估看不到全貌** — ingest 後新增 verify item 尚未出現；user 對 round 2 全 OK 可能誤判「change 全完成」→ 主線 orchestrator 接 archive trigger 提前歸檔
...
review:ui (round N) → user 留 N 條 issue
  ↓
主線 orchestrator triage 每條 issue 到 (A) UX/copy / (B) Behavior / (C) Spec gap
  ↓
...
- `archive-commit-order` — 收尾順序唯一真相：先 archive 再單一 commit；本 § sequence 對齊此
