...
→ 次級 warn。落地 **MUST relay 給該 consumer 的 session**（per [[clade-role-and-todo-discipline]]
§ Consumer 工作命中時 MUST relay；brief 只給 findings + 本 rule pointer + 驗收標準，**NEVER** 替它
拆步驟）。**NEVER** 讓義務停在「出了 warn」——那不會讓任何 session 去修它。
