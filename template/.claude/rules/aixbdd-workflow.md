...
照上游 README 的快速開始九步，**MUST 依序**，不可跳步：

| # | 入口 | 誰執行 | 產出落點 |
| --- | --- | --- | --- |
| 1 | `/constitution` | 共同 | `.agents/constitution/**`（只在要新增或調整 artifact 規則時） |
| 2 | `/specify` | PM | `specs/plans/NNN-<slug>/{spec.md,checklists/requirements.md,truth-delta.md}` |
| 3 | `/clarify-over-specs` | PM | 更新 `spec.md`（選用） |
| 4a | `/spec-by-example` → `/ui-plan` | PM | plan package 的 `features/acceptance/**` 與 `ui-plan.md` ＋ 靜態雛形 |
| 4b | `/technical-research` | RD | plan `research.md` ＋ truth `specs/truth/techstack.md` |
| 5 | PM 確認 Gherkin 與雛形後 handoff | PM → RD | — |
| 6 | `/system-analysis`（委派 `/api-plan`、`/data-plan`） | RD | plan `plan.md` |
...
1. **NEVER 在 plan package 之外寫 acceptance feature**。驗收 Gherkin 屬於 `specs/plans/NNN-<slug>/features/acceptance/**`；拆解後的可執行 interface feature 才進 `specs/truth/features/**`。兩者寫反的症狀不是報錯，是 `specs/truth/` 逐漸累積成一份沒有人維護的需求史。
...
| 反模式 | 為何錯 | 正解 |
| --- | --- | --- |
| 小改動直接改舊 `specs/plans/003-.../spec.md` | 舊 plan 是歷史，改掉之後沒有任何地方看得出這次改了什麼 | `/specify` 開 `004-<slug>` |
| `/specify` 順手改 `specs/truth/contracts/openapi.yaml` | truth 有 owner；非 owner 的寫入下一輪會被覆蓋 | 在 `truth-delta.md` 記 ADD / MODIFY / DELETE 意圖，交給 owner skill |
