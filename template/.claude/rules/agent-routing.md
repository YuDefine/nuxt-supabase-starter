...
| 〔`screenshot-review-verify`〕 | **本列 NEVER 派 Pi 任一 model**——`grok-xai` / `grok-cursor` / `astra` / `gemini` / `luna` 一律不准，`pi-dispatch-screenshot-verify.ts` 已 fail-closed 拒跑。**Cursor runtime 例外**：NEVER 用 target-native task catalog 的 `model=claude-*` 假裝本列；改走 § Runtime residency and native transport 的 Herdr create-only `cc`／`ccw`。**NEVER 恢復「subagent 再轉派」**的形狀。 |
