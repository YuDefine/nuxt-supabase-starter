...
| **Spectra `propose` 階段（draft）** | **預設 Codex GPT-5.5 xhigh draft，無 A/B 詢問** | draft + cross-check 比擇一穩。詳見 `spectra-propose` Step 0。 |
| **Spectra `propose` cross-check** | **主線 Claude Opus 4.8 xhigh** | 主線 = quality gate，不只是 dispatcher。 |
...
2. **MUST** 預設走「Codex draft + 主線 cross-check」流程（步驟與模板見 reference § Codex 派工的標準流程）
