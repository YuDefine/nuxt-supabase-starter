...
# OPSX Workflow — Cross-Phase Ownership（治理 doc）
...
「key 會     「render 後沒          「資料形狀健全嗎          「5 維度跨檢            「archive 前
 render 嗎」   整列 fallback 嗎」      (param vs schema)」        (主線 + qualified checker)」  再驗一次」
        ╰────────────── Layer D（UI-INVARIANTS）是上面 B/C/E 共同引用的宣告式契約 ──────────────╯
```

## Executor and capability boundary

這張 matrix 定義 ownership、predicate 與 evidence layer；每一個 phase 的實際執行者、model、effort、runtime 與可用工具，MUST 在派工前重新讀當前已核准的 phase policy（目前 routing／qualification 的 SoT 見 `[[agent-routing.pi-watch-protocol]]` 與 `[[agent-routing.routing-table]]`）。本檔保留歷史案例與工具協定，**NEVER** 把 matrix 的 executor 文字讀成三個產品入口都已實測或都具備相同 loader／browser／dispatch 能力。

`page-display-check.ts`、`refactor-invariant-check.ts`、`audit-data-sanity.ts` 與 verify channel 是各 layer 的既有工具／協定名稱；目前 Layer A 的 verified helper 是 `vendor/scripts/spectra-advanced/page-display-check.ts`。工具的實際載體、讀取入口與完成 receipt 仍由當前 phase policy 及 target adapter 判定。工具名稱存在、文件投影成功或一個 runtime 的案例通過，都不替其他 runtime 產生支援證據。

## Responsibility Matrix

| Phase / checkpoint | **負責抓**（owns） | **不負責**（defers to） | 機制 / Layer |
| --- | --- | --- | --- |
| **OPSX intent** | manual-review item data-readiness：sample key inline + **該 key 是否真的會在 target UI render**（reverse page-grep） | runtime 正確性、視覺 | **Layer A** `VERIFY_UI_SAMPLE_KEY_DISPLAY_CHECK` + `vendor/scripts/spectra-advanced/page-display-check.ts` |
| **OPSX execution — implementation** | code 正確性、contract 遵循、typecheck/lint/unit | UI render 結果、視覺、跨檔資料形狀 | typecheck / lint / tests |
| **OPSX execution — UI view** | refactor invariant：admin list/table **無 column 整欄塌縮成 fallback** + page load 0 個 4xx/5xx | static 資料形狀、主觀視覺 | **Layer B** `refactor-invariant-check.ts` |
| **OPSX execution — design review** | 資料形狀 sanity：client query param literal vs server zod bound + lookup-map empty risk | runtime render（B 管）、視覺 | **Layer C** `audit-data-sanity.ts`（`/data-sanity`） |
| **OPSX execution — verify:ui** | DOM observation evidence：screenshot 真的支撐 `dom=` 宣稱（誠實，不 fab） | 跨維度 batch 檢查（E 管） | verify channel；fab 由 E.2 D2 batch 擋（write-time guard = 未來 3.2） |
| **OPSX execution — pre-handoff** | 5 維度跨檢（task↔render / evidence↔dom fab / list↔fallback / api boundary / error tail），主線與當前 phase policy 指定的 qualified checker | — | **Layer E.1**（主線 self-analysis）+ **E.2**（qualified cross-check；target adapter 必須提供 receipt） |
| **OPSX archive** | archive 前再跑 Layer C data-sanity + archive-gate Check 1–5 | — | `archive-gate.sh` + **Layer C** re-run |
...
| Step 8a verify:ui | 主線從 task description synth `dom=EMP-007-approved` annotation，無 grep-against-screenshot 校驗 | **Layer E** D2 fab guard：qualified checker 讀 screenshot 比對 `dom=`，screenshot 不支撐 → block + `（issue:）` |
| Manual review handoff | review-gui 直接把 9 個帶 fab annotation 推給 user，無 pre-handoff cross-check | **Layer E.1 + E.2** Step 8a.6：handoff 前主線 + qualified checker 雙閘 5 維度 |

## 各 phase SKILL 的 ownership 宣告

OPSX 的 `references/intent.md` 與 `references/execution.md` 承載目前流程；本表保留各檢查的 owner。每次修改對應 phase 時 **MUST** 核对該列的既有品質契約。OPSX archive 由 `opsx-control` 呼叫既有 gate，並額外核對目前 revision、projection 與 active lease；不能以改入口為由省略 UI／資料檢查。
...
- clade 自身**不使用 spectra**（走 plan mode），本檔 path-scope `openspec/changes/**`，
  只在 consumer 處理 change artifacts 時載入。

## Runtime evidence status

各 target adapter 提供 OPSX execution、dispatch、browser 或 receipt 的實際入口；本共通檔承載跨端不變的 ownership 與 evidence contract。未有 target-specific event／tool／completion receipt 時，該 layer 仍保持未完成。
