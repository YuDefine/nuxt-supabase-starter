...
2. **clade aggregator**（`scripts/sync-consumer-meta.ts`）從各 consumer 拉 meta，cross-validate 對應 `package.json` / `wrangler.toml` / `nuxt.config.ts`，generated fields 帶 `source: <file>` provenance
...
`node scripts/sync-consumer-meta.ts`：

1. 讀 `registry/consumers.json` 取得各 consumer 的 path（透過 `registry/consumers.local.json` 解析本機 absolute path，方式同 propagate.ts）
2. 對每個 consumer：
...
```bash
cd ~/offline/clade
node scripts/audit-consumer-meta-adoption.ts              # markdown report (default)
node scripts/audit-consumer-meta-adoption.ts --json       # machine-readable
node scripts/audit-consumer-meta-adoption.ts --consumer <abs-path>   # single
```
...
Diagnostic-only（exit code 0 always）。**不**自動 wire 進 `propagate.ts` post-audit hook — consumer 自己跑 / clade session 用，不強制散播時跑。

### `scripts/scaffold-consumer-meta.ts` — 提議單一 consumer 的 manifest 內容

```bash
cd ~/offline/clade
node scripts/scaffold-consumer-meta.ts ~/offline/<consumer>            # markdown report + JSON block
node scripts/scaffold-consumer-meta.ts ~/offline/<consumer> --json     # JSON only (pipeable)
```
