...
當 `proposal.md` 宣告 `**No user-facing journey (backend-only)**` 時，`## 人工檢查` 區塊適用更嚴的規約 — **只**允許三類項目（production 授權 / 商業判斷 / production 觀察），其餘 SSH / psql / curl / schema 驗證等 evidence collection **MUST** 寫進新的 `## N. Backend Verification Evidence` section 由 apply 階段 session owner 自跑自貼，**禁止**塞進 `## 人工檢查` 讓使用者扛。
...
**Why**：baseline 維護不是任何單一 work item 的臨時工作。每次 verify dispatch 才問 user 或讓 agent 補 seed，會把長期 codebase baseline 拖進錯誤時機，且會製造 seed.sql source-of-truth 漂移。

**Agent 端對應行為**：target visual verifier verify mode 撞 baseline 缺（dev-login route 不存在 / seed 缺 entity / known URL 只能呈現空資料）屬 Fail-Fast UNCERTAIN；agent **NEVER** 補 seed、patch auth、或升級成 mutation runner。

**主線端對應行為**：

- Dispatch `verify:e2e` 前 **MUST** 確認 reproducible browser runner config、dev-login / `__test-login`、three-role fixture 存在。
...
然後結束」——本行 2026-08-28 之前正是那樣寫的（`Track via docs/tech-debt.md（當前 entry: TD-158）`），
而 TD-158 現在 grep 不到：登記型停手的接手機制斷掉時，斷掉本身不會發出任何聲音。
...
`[discuss]` items 由交付前收尾 walkthrough 觸發（[[manual-review]] § `[discuss]` walkthrough）；主線 session owner 主動準備 evidence（含 backend grep / query / SSH / curl / migration check 結果）→ 使用者明確 OK / Issue / Skip / Defer。

完整 spec（七步 walkthrough、Defer-to-HANDOFF schema、`## Deferred discuss items` HTML marker schema、混合 kind 的執行順序）：詳見 [[manual-review]] § `[discuss]` walkthrough。
