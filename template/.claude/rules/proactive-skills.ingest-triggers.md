...
`spectra-ingest` 是 apply 階段的「需求漂移補丁」。當 proposal / tasks / design artifact 與實際需求或實作現況出現結構性落差時，**orchestrator MUST 主動引導使用者**（不是等使用者自己想起）。
...
`post-edit-drift-check.sh`（hook）會自動偵測 5、4、2 的部分靜默漂移，寫 stderr 提示 orchestrator 考慮 ingest。LLM 判斷層面則需要主動感知 1、3、6。

### 決策規則（明確直接做、模糊再詢問）

| 情況 | 動作 |
| --- | --- |
| 信號明確（migration 新增未紀錄欄位、使用者直白改需求、journey 遺漏具體 URL） | orchestrator **直接跑** `spectra-ingest`，口頭告知「偵測到 X，已觸發 ingest 更新 Y」 |
...
理由：ingest 會新增 verify items，這些 item 在 apply 落地前無 evidence；user 對中間態評估會誤判「change 全完成」、主線 orchestrator 接 archive trigger 提前歸檔；兩段 commit 也讓 spec 跟 code 暫時不同步。詳見 [[manual-review]] § 人工檢查時機。
...
- reference impl：<consumer-a> / <consumer-b> / <consumer-d> / <consumer-i> 既有 dev-login route

完整 hard rule（含 scaffold gate template、runtime-supported input surface 或直接對話的兩選項）見 [[manual-review]] § Pre-verify baseline 假設 + Dev-login route missing → scaffold-first。對應 [[pitfall-agent-asks-user-cookie-skipping-dev-login-scaffold]]。
...
**Drift 不自動改寫 MANUAL**（避免誤刪人寫內容），以 stderr 警告提示。orchestrator 看到
...
| 在外部 runtime（背景 codex / 非 另一個 runtime session）跑 spectra command 之後 | **你必須手動** `pnpm spectra:roadmap && pnpm spectra:claims` | 外部 runtime 不觸發 PostToolUse hook |
...
### orchestrator 主動維護的時機

orchestrator **必須**在以下時機更新 `## Next Moves` 區塊（MANUAL block）：
