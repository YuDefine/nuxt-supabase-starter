...
**自動產出具有 WIP 的所有權保護**：hook / sync-vendor / spectra-archive 的內容同樣需要查明來源。已驗證為本次 hook 的 index 殘留時，走 [[commit]] 的 `MM` recipe；其他 automation 產物依歸屬與既有授權處理。**禁止**只因「這違反 X rule 應該還原」就處置；無法確認的內容保留，不自行判成可丟棄。
...
- **A. 馬上修，回 flow 繼續** — 在當前 session 直接編輯該 WIP 檔，原 flow（`/commit` / `/spectra-archive` / 等）繼續走完。前提：A0 已送出且對方不反對（或 A0 不適用），且 user 確認該編輯不會跟別 session 的修改互踩（或別 session 已結束）
