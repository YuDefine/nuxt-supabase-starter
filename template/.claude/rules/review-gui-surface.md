...
**Iron Law：`[verify:ui]` / `[review:ui]` 的 evidence 一律由 `screenshot-review` reviewed visual-evidence worker
收，主線只消費它回的 JSON 摘要。主線 `Read` 截圖是例外路徑，只在下表命中時開放。**

**這個 channel NEVER 外派給 unreviewed carrier。** 逐字包含：**NEVER** 派 Pi 任一 model
（`grok-xai` / `grok-cursor` / `astra` / `sol` / `gemini` / `luna` 一律不准）、**NEVER** 走
...
§ 為什麼只准 reviewed visual-evidence worker。
...
| 可觀察 predicate | MUST |
| --- | --- |
| 要對 `[verify:ui]` / `[review:ui]` item 收 evidence | 派 `screenshot-review` reviewed visual-evidence worker（唯一入口），主線只消費它回的 JSON 摘要 |
...
**NEVER** 把本節讀成「reviewed visual-evidence worker 比較可靠所以選它」：[[pitfall-screenshot-review-sonnet-wrapper-self-rationalize]]
