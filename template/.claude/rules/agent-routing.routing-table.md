...
**Cursor Grok 主線不執行本表「由誰執行」欄。** 列名／slug 共用。載體 remap 見 Cursor User Rules「Cursor 主線 routing」與 `vendor/cursor-rules/cursor-model-residency.mdc`。**NEVER** 把本表讀成 Cursor Task 換 model。
...
| `--model sol --effort xhigh` | Nuxt 本體實作（`nuxt-core-implementation`） |
...
| Claude Opus 5（effort: medium） | 其餘 UI view 實作（非 Nuxt UI／Content、非 Nuxt 本體）；Cursor → Herdr cc／ccw；Claude Code native Agent。GPT／Pi 不承接這類 UI |
| Cursor Task `composer-2.5-fast` | 僅 Nuxt UI 元件組裝與 Nuxt Content；不是 Pi、也不是 Herdr |
| Cursor 主線 Grok 4.6 + `cursor-ide-browser` | Design Review／視覺判讀。Claude Code 維持合格視覺 executor，不走 Pi Grok |
...
| 〔`web-search`〕 **External web retrieval**（`WebSearch`／`WebFetch`） | **Pi bare `gemini high` → linked `luna high` → matching receipt 才放行同種 built-in tool** | 兩支工具共用 machine row；查不到就回「查不到」。 **本列另有硬禁令**，見 [[agent-routing]] § Routing 硬禁令。 |
...
| 〔`screenshot-review-verify`〕 **`screenshot-review` 全部模式**（`[verify:ui]` channel / archive 前視覺 QA / commit 0-B / ad-hoc 截圖） | **Pi `--model gemini --effort high` via 泛用 dispatcher**。exit 4 → `--model luna` 同 effort（不進 grok）。**NEVER** Cursor Task `model=claude-*` 假裝本列；**NEVER** 恢復「subagent 再轉派」 | 2026-09-08 Charles 拍板改回 Pi Gemini 3.8 Flash high（推翻 2026-08-22 Claude-only）。機械取證走本列；Design Review／視覺判讀不是本列（Cursor 主線 Grok + `cursor-ide-browser`；Claude Code 合格視覺 executor）。 **本列另有硬禁令**，見 [[agent-routing]] § Routing 硬禁令。 |
| 〔`nuxt-core-implementation`〕 **Nuxt 本體**（`nuxt.config`、pages／layouts 結構、routing、server／Nitro、plugins、composables、middleware、data fetching 契約） | **Pi `--model sol --effort xhigh` via 泛用 dispatcher** | 不是 Nuxt UI／Content（Cursor Task Composer 2.5），也不是其餘 UI view（Opus 5（effort: medium））。mutation。 |
| 〔`ui-view-implementation`〕 **其餘 UI view 實作**（非 Nuxt UI／Content、非 Nuxt 本體） | **Claude Opus 5（effort: medium）**：Claude Code native Agent；Cursor → Herdr create-only `cc`／`ccw`（`--model claude-opus-5 --effort medium`）。**本列 NEVER 派 Pi** | Design Review／視覺判讀不是本列。 |
