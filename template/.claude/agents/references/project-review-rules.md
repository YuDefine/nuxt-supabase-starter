...
| `package.json` 新增或保留 `consola` runtime dependency                                      | 移除；用 evlog 的 request / standalone / pipeline API                                  | evlog 已提供 `useLogger(event)`、`createLogger()`、`createRequestLogger()` 與 `createDrainPipeline()`；不需要多裝 consola 當 fallback。                                |
...
5. `grep -rn "from 'consola'" server/ package.json` — 不應出現
