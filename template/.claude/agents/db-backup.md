...
可能原因:
- DB 連線失敗（local Docker 模式：`supabase status` 檢查；remote LXC 模式：SSH / Tailscale 連線檢查；Cloud：access token 過期 / project 已 pause）
- 磁碟空間不足
- ...
...
- 執行前不需要確認，直接執行
- 連線失敗回報錯誤並依 consumer 的 db-runtime 變體（local / self-hosted / cloud）建議對應檢查路徑
- 備份完成後不要自動刪除舊備份
