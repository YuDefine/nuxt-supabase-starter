...
6. worker brief（`fanout` 的每一份）**MUST** 寫明：做完自己那段卻留下殘工時，把殘工另寫一份 durable
   ...
   另 **MUST** 寫明切片契約（與 Cursor Project `CreateAgent` 同一結果）：自己的 branch、非空 committed diff 後開 **draft PR**、盯該 PR 的 CI、紅燈修同一張 PR、**NEVER** merge／**NEVER** 直推 `main`。要 live shared DB 才 desk；隔離雲端 VM 需要 DB 時起該 VM 的 ephemeral 實例（[[db-topology-invariant]]）。Worker `--complete success` **不是** landing；coordinator 收件後才 `batch ready`／完整 `/commit`／條件式 `merge-unattended`。
