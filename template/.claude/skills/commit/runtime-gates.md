...
| 0-A.1 | 合格獨立 reviewer（GPT-6 Astra via Pi，effort: medium）；主線核對資格與結果 | 完整 verdict 與全部 finding；缺資格、缺覆蓋或 snapshot 漂移時未通過 | review-policy 各欄、真實 dispatch／session、完整輸出 |
| 0-A.2 | Critical／Major 觸發時由同一個 Astra medium 以 fresh context 對修復後 snapshot 深度複審 | 深度 review 完成才通過；reviewer 不可用或配額耗盡時保持未完成，不是合法 skip | 深度 reviewer 身份、snapshot／verdict、逐條 finding 處置與修正依據 |
