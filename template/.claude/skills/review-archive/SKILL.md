...
共同來源 § 向使用者提問 的三條義務，在 Claude Code 用 `AskUserQuestion` tool 實作：選項寫進 `options`（含「跳過」那一項），拿到回覆才往下做。

**`AskUserQuestion` 是這三條在本 runtime 的載體，NEVER 是三條本身。** 工具不可用時義務不消失——改用其他方式問，**NEVER** 讀成「問不了所以可以靜默跳過」。
