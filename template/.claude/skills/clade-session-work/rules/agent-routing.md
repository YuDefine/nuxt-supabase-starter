...
**核心命題**：Nuxt UI／Nuxt Content 實作使用 Cursor Composer 2.5，Nuxt 本體使用 GPT-5.6 Sol xhigh，其餘 UI view 實作使用 Claude Opus 5（effort: medium）；Design Review、UI 詳細計畫與截圖項目符合性判定使用 Claude Opus 5；screenshot review 使用 Gemini 3.8 Flash。一般非 UI 用 GPT-5.6 Luna／Gemini 3.8 Flash、複雜非 UI 用 GPT-5.6 Sol；GPT-6 Astra 只做非 UI 計畫、決策與 code review。本規則優先於 skill 內嵌指示。
...
>
> **決定要派 pi 之後、送出 dispatch 之前，MUST 先 Read reference 的 § Pi 派工的標準流程。** reference 的 `paths:` 綁的是 工作計畫 / screenshot 情境，「單純要派 pi」的 session **不會**自動載入它——靠 auto-load 會讀不到（成因與實測見 `docs/rule-rationale/agent-routing.md`）。
...
**不外派**（命中就自己做，即使同時有好幾條）：少量工具即可結束；路徑已知的少量讀取；規約 / 契約 / 對外文件的**定稿措辭**（會原樣成為交付物的最終文字）；已符合具名 UI 列模型與工具資格的主線工作（Nuxt UI／Content：Composer 2.5；Nuxt 本體：Sol xhigh；其餘 UI view：Opus 5；Design Review／UI 計畫／截圖符合性：Opus 5；模型不符時走 bounded phase transport）；需要 claude.ai-connected MCP、且 **ntn CLI 做不到** 的工作（Notion 走 〔`notion-ops`〕，不在此列）；安全敏感或不可逆的動作（憑證 / 刪檔 / force push / 對外發佈）；**複驗自己剛做完的東西**（判準見 [[checker-subagent]] § 過度派）。

**具名 threshold 優先於上段的 generic「少量」判準**：同一 prompt segment 準備執行第 3 個高信心 readonly Bash、第 5 個 distinct textual Read，或第一次 Read 501+ 行文字檔時，PreToolUse gate 會建立 pending decision。主線 MUST 以 decision-linked dispatch（exact trigger 依 `mechanical-fanout`／`read-heavy-scan` 走 Gemini 3.8 Flash high；更具體工作可走共用 policy 驗證過的 concrete Pi row）、structured waiver，或 dispatcher exit 3／4 後的 matching fallback authorization receipt 結案；完整命令與狀態轉移見 reference § Routing threshold gate。
...
## 派工執行期（全文在 [[agent-routing.dispatch-execution]]，具名時機 MUST-Read）

上節決定**派不派**。**決定要把某件工作交出去之後、寫出任何一份 dispatch brief 之前，MUST 先讀
[[agent-routing.dispatch-execution]]**——沒讀到就沒有 § 派多少 的四步 brief 範圍判定（要切在串行鏈
中間，MUST 具名指出留下的第一環需要主線或 user 的什麼；講不出來就整條交出去）、沒有
§ Subagent 回報契約 的 4-status 收尾要求與「report 是未驗證主張」的 scope 核實紀律、沒有
§ Plan mode 期間的派工邊界 的可派／不可派逐列名單，也沒有 § 配額邊界 撞 dispatcher exit 4 時
**payload 算不出來**的那三條判斷。

**收到 subagent／pi 回報的那一刻、以及 dispatcher 回 exit 4 的那一刻**，同一份檔的
§ Subagent 回報契約 與 § 配額耗盡時的 fallback 紀律 是那兩個時刻的 MUST-Read，**此處不複述**。

該檔的 `paths:` 只綁 pi dispatcher／routing gate 的 script 與 agent 定義檔（3%–12% session 命中），
**「單純要派工」的 session 不會自動載入它**——與上方 reference 同一個成因，靠 auto-load 會讀不到。
...
**UI 角色與 session residency 分開判**：每一個實作 phase 先分三類：Nuxt UI／Content 交 Cursor 原生 Composer 2.5，Nuxt 本體交 GPT-5.6 Sol xhigh，其餘 UI view 交 Claude Opus 5（effort: medium）；每一個 Design Review、UI 詳細計畫、截圖項目符合性判定交給 Claude Opus 5；四種 screenshot review 模式一律交給 Pi Gemini 3.8 Flash。具名列見 [[agent-routing.routing-table]]。

當前主線符合該列模型且具備實際工具時可原地執行；否則 change carrier 保持原 session，只交出該 bounded phase。Composer 使用當次 Cursor catalog 已提供的 Composer 2.5 model ID；Claude 使用 Claude Code 原生／Herdr carrier；Gemini 使用既有 Pi provider。Opus 5 無法執行 Design Review、UI 詳細計畫或截圖符合性時，帶實際失敗／不可用證據走原列的 GPT-5.6 Sol（effort: high）fallback；Sol 也無法執行才保持未完成。其餘 UI view 實作的 Opus、Composer 與 Gemini 不可用時保留 blocker，不套用三種 review／planning 工作的 fallback。Pi Cursor 配額池的 mutation 禁令維持，Cursor 原生 UI 實作不透過該配額池。截圖收集與項目符合性判定分兩次 dispatch。
...
**逐列硬禁令表（9 列）、`--route`／`--tier-basis`／`--table-row` 三條必填、Cursor sandbox 與
workspace-access 的四段判準，全文在 [[agent-routing.routing-table]] § Pi 派工的 model 維與
workspace capability ＋ § Routing 硬禁令**——它與對照表同一個檔，查列時本來就同時被消費。
**NEVER** 只讀對照表就派：列的 model 與列的禁令是兩件事，只讀前者與沒查表事後不可區分。

### Plan mode 期間的派工邊界在 [[agent-routing.dispatch-execution]]（具名時機 MUST-Read）

**在 plan mode 決定派不派任一 Routing Table 列之前，MUST 先讀
[[agent-routing.dispatch-execution]] § Plan mode 期間的派工邊界**——ledger 與 prompt 寫在 homedir
故不違反 no-mutation 契約、可派與不可派的逐列名單、以及 `pi-routing-gate.ts` 對 plan mode 的放行
與唯一可跑的 `[waive] --reason plan-mode-readonly`，都在那裡。**NEVER** 因為「plan mode 不該有
副作用」就把 read-only 的派工也退回主線自己做（TD-507）；**NEVER** 為了走 `[dispatch]` 而在
plan mode 寫 brief 檔繞過 harness，**也 NEVER** 把該 latch 回報成 deadlock。
...
1. **NEVER** 直接呼叫 Claude Code 內建的 `WebSearch` 或 `WebFetch`；照 gate 印出的 decision-linked 指令依序走 bare `gemini high` → bare `luna low` → matching receipt 才放行原tool kind與candidate。完整旗標、exit transition與receipt契約見 [[agent-routing.pi-watch-protocol]] § External web retrieval。
...
**八列 Dispatch 入口禁令表全文在 [[agent-routing.dispatch-execution]] § 必禁事項 — Dispatch 入口**
——派任何 pi／subagent 之前 MUST 先讀那一節，**NEVER** 憑印象認定自己這次沒命中任何一列。

**派 pi 寫 code、或收 `verify:ui` evidence 之前，MUST 先讀 [[agent-routing.pi-watch-protocol]] § Dispatch 入口禁令（下推四列）**——Commit Authorization 段、Plan-first 硬指令、`verify:ui` 唯一入口與它的機械 backstop、以及「pi MUST 由該層編排者自己派」那條的准入與範圍界線，都在那裡，**此處不複述**。
