...
1. **檢查人工檢查狀態（kind-aware）** — 讀取 tasks artifact 的 `## 人工檢查` 區塊，依每條 item 的 leading kind marker 分流。
   ...
   - **若 `unchecked_review_ui` > 0 且 `unchecked_discuss` > 0**（混合 kind）：依序執行 — 先跑 spectra-archive 內建 Step 3.5「Discuss Items Walkthrough」（讓 Claude 主動準備 evidence 與使用者討論並寫 `(claude-discussed: <ISO>)` annotation），再用 AskUserQuestion 提示是否跑 `/review-screenshot` 處理 `[review:ui]` items。順序：discuss 先、review:ui 後 — 讓使用者見過所有討論點再進視覺驗收。
