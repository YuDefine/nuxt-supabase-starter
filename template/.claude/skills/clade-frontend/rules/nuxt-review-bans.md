...
**核心命題**：`vendor/review-rules/patterns.json` 定義了跨 consumer 統一的機械可檢 ban 規則，由 pre-commit hook + code-review agent 消費。review 端的規則檔（`capabilities/core/agents/references/clade-review-rules.md`）只有 code-review agent 讀；實作階段看不到 ban 就會反覆寫出違規、事後才被抓，甚至在 `app.config.ts` 用 `defaultVariants` 繞過只掃 `.vue` 的機械層。
