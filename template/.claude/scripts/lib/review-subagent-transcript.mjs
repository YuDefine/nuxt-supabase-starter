...
//      那份 review（exit 6，2026-09-24 0-A Major）
...
/** requested 是完整 model id（claude-opus-5-5）或 family alias（fable）；alias 比前綴。 */
...
/** harness 可能在 user message 旁掛 system-reminder；比對 prompt 時剝掉它們。 */
...
export function verifySubagentReview({ subagentsDir, nonce, prompt, brief, model, agentType }) {
  ...
  const sent = stripSystemReminders(firstUserText(readTranscript(transcript)))
