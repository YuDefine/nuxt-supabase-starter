...
| 禁止使用 | 應替換為 | 說明 |
| --- | --- | --- |
| `<UBadge size="xs">` / `<UBadge size="sm">` | `<UBadge size="md">`，或移除 `size` prop（吃預設值） | badge 用 `xs` / `sm` 在密集表格與狀態欄字級過小、可讀性不足。`<UBadge>` 的 `size` 預設即 `md`，故 `md` 是最小可接受基線；未寫 `size` 的 callsite 吃預設值、合規不需動。（per nuxt-ui-remote MCP：`size` 列舉 `xs｜sm｜md｜lg｜xl`，預設 `md`） |
...
- 無業務例外：`<UBadge>` 一律最小 `md`，密集 UI 需更小視覺密度時改用其他呈現（如純文字 + `text-muted`），不得降 badge size
