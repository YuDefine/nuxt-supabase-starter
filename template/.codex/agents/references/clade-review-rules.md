...
| `app/**/*.vue`、`packages/*/app/**/*.vue`、`components/**/*.vue`、`layouts/**/*.vue`、`pages/**/*.vue` | Nuxt a11y 採用一致性、元件替代規則、UI 元件尺寸下限、Dark Mode、Form 驗證模式 |
...
4. `grep -rEn '<(img|image)\b' app/ packages/*/app/ components/ layouts/ pages/ 2>/dev/null | grep -v "raw-img"` — 找未標記例外的原生 `<img>`（用 `\b` 而非 `\s` 才能涵蓋多行 `<img\n  :src=...>` 與 self-close `<img>`）
...
| 禁止使用 | 應替換為 | 說明 |
| --- | --- | --- |
| `<USlideover>` / `<UModal>` / `<UDrawer>` 的 **default slot** 放 body 內容 | 內容放 `#body` slot；搭配 `title` prop + 內建 close button 取代手寫 `#header` close | Nuxt UI v3 的 overlay 元件 default slot 語意是 **trigger**（開啟 overlay 的按鈕），`#body` slot 才有內建 `overflow-y-auto`。body 內容放 default slot → 超出 viewport 高度時**無法捲動**（scroll-y 壞掉）。此問題在 <consumer-b> 專案反映超過 5 次、跨多頁面重複犯，是最高頻的 Nuxt UI 誤用。 |
...
| 禁止使用 | 應替換為 | 說明 |
| --- | --- | --- |
| `<UBadge size="xs">` / `<UBadge size="sm">` | `<UBadge size="md">`、`<UBadge size="lg">`、`<UBadge size="xl">`，或移除 `size` prop（吃預設值 `md`） | `<UBadge>` 的 `size` 只允許 `md` / `lg` / `xl`。`xs` / `sm` 在密集表格與狀態欄字級過小、可讀性不足。未寫 `size` 的 callsite 吃預設值 `md`，合規不需動。（per nuxt-ui-remote MCP：`size` 列舉 `xs｜sm｜md｜lg｜xl`，預設 `md`） |

**Reviewer 檢查方式**：

1. `grep -rEn '<UBadge[^>]*size="(xs|sm)"' app/ packages/*/app/ components/ layouts/ pages/ 2>/dev/null` — 找單行 `<UBadge>` 使用 `xs` / `sm`（`[^>]*` 限同一 tag 內，不誤抓 `<UButton>` / `<UAvatar>` 等其他元件的 size）
...
| `handleDbError(error)` 後沒 `throw`（<consumer-a> 等專案的 helper returns，不 throw）              | `const r = handleDbError(error); throw createError({ status: r.statusCode, ... })`      | helper 設計是 returns，缺 `throw` 會讓錯誤被吞掉，handler 繼續往下跑。是 evlog 採用後最常見漏網情境之一。                                                              |
