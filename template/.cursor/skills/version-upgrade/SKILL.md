...
Resolve `<skills-root>` and `<native-skills>` to `.cursor/skills/`. The shared `npx skills` CLI remains the installer and uses the commands in the common workflow. Retain the common Grok Pi mutation chain and never route mutation to a `*-cursor` readonly pool. If the native Cursor host lacks a verified bounded background/completion surface, stop the unattended branch and report the gap; use the active conversation for user questions.

Runtime bindings: `<runtime-target>` is `cursor`; `<runtime-agent>` is `cursor` as shown by the installed shared CLI help.
