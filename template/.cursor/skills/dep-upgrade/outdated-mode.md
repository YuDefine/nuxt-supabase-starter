...
| Plan section 缺 / commit message format 不符 / scope drift | pi 漏跑硬指令 | 不升 research；直接 AskUserQuestion [重派 first-pass（同 `--effort low`）/ 升 research / 跳過 / 中止] |
| `fetch failed` / sandbox 拒絕 / 互動 prompt 卡住 | 環境問題 | per watch protocol 「介入觸發」，AskUserQuestion |
...
4. 如果改動涉及多 step 聯動（如 `upload-artifact` 的 output 被後續 step 引用）→ **STOP + AskUserQuestion** 讓 user 拍板
...
1. AskUserQuestion 讓 user 選：立刻回 worktree 修 / revert 該 action 的 commit / 登記 HANDOFF
2. 修完後再 push → 再派 watcher 驗綠
3. **NEVER** 對 CI 紅燈視而不見宣稱 dep-upgrade 完成
