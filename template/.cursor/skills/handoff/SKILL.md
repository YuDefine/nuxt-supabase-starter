...
Cursor mainline keeps `park` available but relay and fanout are unsupported; never open a successor pane for those args. Use the active conversation for questions and explicit drift checks. A clean successor may use create-only Herdr transport for a Cursor workspace only when launcher, workspace, label, and identity preflight pass; a native Task/Agent is bounded execution and cannot substitute for durable handoff. Missing background/completion evidence blocks only the dependent dispatch.
