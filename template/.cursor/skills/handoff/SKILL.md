...
「Session」=**當前這個 chat session**，不是 working tree / git state / 檔案系統狀態。User 經常並行多開 Claude Code session 工作，所以 `git status` 髒污、`tasks/<date>-*.md` 內 unchecked 項、plan package `tasks.md` 的 unchecked 項 **都可能來自別的 session**，不能拿來判斷當前 session 是否有未交辦工作。
...
3. **寫入**：依分類 Edit / Write 對應檔案，path **MUST** 用 Step 1.5 解析出的 `$MAIN_WT_PATH/<rel>` 絕對路徑（即使當前 cwd 在 linked worktree）。格式與落點判準走下方 § HANDOFF 寫回契約（三條，寫入前逐條過）。HANDOFF.md `## In Progress` 條目 MUST 含：
   ...
   - **若來自 [[worktree-default]] §8 死鎖**：額外加 Stop hook 攔點摘要、missing acceptance criterion、改過檔案的 selective stash ref（若有，例 `stash@{0}: <slug>-handoff`）、下一 session 接手指引（指名 carrier 路徑與剩下的 phase；實作先隔離 worktree，收尾先驗當前 evidence 與人的 gate）
...
`mergeBackSafety` 三 signal 的推導與處置表、9 列 kind 判定表（含 `specs/plans/` 不存在的 repo
...
禁止 dispatch 收尾工作，走 § 2B.4.5 分流。stash 對 `raw.stashes[*]` **每一筆**都
...
Cursor mainline keeps `park` available but relay and fanout are unsupported; never open a successor pane for those args. Parallel slices use Project **CreateAgent** (one agent per independent slice). Each implementer opens a **draft** PR on its own branch, registers `batch draft --kind visibility`, subscribes to that PR's CI, and returns CI-red fixes to the same PR. Slice **worker NEVER merge**. Worker completion MUST return durable receipts (`workId`, repository, PR, branch, checkpoint SHA, scope, evidence, writer-release) and then stop writing the source. The named coordinator receives those receipts, runs `/commit` on a local managed source, and may squash only when unattended predicates hold. Charles does not open a pane or paste a prompt to continue. A native Task/Agent is bounded execution and cannot substitute for durable handoff. Missing background/completion evidence blocks only the dependent dispatch.

Cursor Project overlay: prefer Devin SWE-2-Max when that catalog slug exists; if the transport is missing, Composer 2.5. Do not invent a Devin row in the routing table. Other runtimes keep the routing table but still owe draft-PR + CI-watch + parallel slices.
