...
`/commit` 在此指共同 commit 流程；入口沒有原生 slash command 時，載入該流程的完整 skill
正文並透過已驗證的 runtime 執行能力完成同一組 gates。沒有流程或所需工具就回報 gate 未完成，
不改用另一套檢查。這裡的 db-runtime variant 指應用資料庫部署方式，與 Claude／Codex／Cursor
無關；選擇 AI runtime 不改變 self-hosted 與 local 的命令路由。下方 pre-commit 為 Git hook，
其已安裝與實跑證據不由原生 agent hook 的設定推論。
