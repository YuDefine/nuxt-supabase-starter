...
**Iron Law：適用 hard tier 是收工線；soft tier 是限制新大工作段。** 超過 soft tier 後，MUST 不開新的 tasks 檔、多檔重構、新的實作 phase 或尚未載入的 skill；手上驗收與小 item 仍可完成。超過 hard tier 後，MUST 保存狀態並按下節交接。壓縮不重設 hard-tier 義務。

**主判準的可觀察 predicate 表、已核准 profile 的兩級語義全文、以及量測口徑紀律（累計用量／當前佔用／壓縮後剩餘不是同一個值；**NEVER** 借另一個 runtime 的門檻或 model 名稱宣稱豁免；門檻 **NEVER** 自行以 env／flag 放寬）在 [[session-tasks.context-budget]]**——由 `session-context-budget-warn.sh` 越過門檻時的提示指過來。
