...
| --- | --- | --- | --- |
| 1 | `rules/core/*.md` | 逐 target 投影到該 runtime 的 rules 目錄（Claude `.cursor/rules/<name>.md`、Codex `.agents/rules/`、Cursor `.cursor/rules/`），無條件收 | 無 `paths:` → always-load；有 `paths:` → 命中才載 |
...
| 只有 clade 自己（治理、散播機械、clade 自治區紀律） | clade 自己的 `.cursor/rules/local/`（clade home 是 Claude session，這格就是字面路徑），**不散播** |
| 只有某一個 consumer，且已是穩定規約 | 該 consumer 自己的中立 local 規約 `.clade/rules/`（legacy `.cursor/rules/local/` 仍可讀，per [[local-rule-override]]），**不進 clade** |
