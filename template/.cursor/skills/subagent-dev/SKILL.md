...
Use Cursor's current task and independent-agent catalog when present. Dispatch the implementer and reviewer with bounded briefs, keep the reviewer read-only, and record the actual model and effort in the receipt. If the catalog or wakeup capability is absent, preserve the work identity and report the boundary; never invoke another runtime as a substitute.
