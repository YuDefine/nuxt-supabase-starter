...
Run the bundled scanner through Cursor's available terminal using the skill-provided `$SKILL_DIR`; keep all source reads and the report output within the requested workspace and never write the blog repository. If terminal execution is unavailable, report that boundary.
