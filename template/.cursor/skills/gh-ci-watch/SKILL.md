...
- Consumer 端（hub-core 投影）：`.cursor/scripts/gh-ci-watch.sh`
...
```bash
bash .cursor/scripts/gh-ci-watch.sh run <run-id>
```
...
- **NEVER** 把「沒收到通知」解讀成任何結論——用 `TaskOutput(block=false)` 查該 task 狀態確認它還活著；**NEVER** 改讀 BashOutput tail（那會把 output 送進 context，在 control-turn allowlist 之外）
