...
**Agent routing**: 若 debug 過程需派 codex 跑診斷 / repro，**MUST** 走 [`rules/core/agent-routing.codex-watch-protocol.md`](../../../../rules/core/agent-routing.codex-watch-protocol.md) § Codex 派工的標準流程 + Codex Watch Protocol；**禁止** `Agent` tool with `agent_type: screenshot-review` 派視覺 QA（per `rules/core/agent-routing.md` Routing Table — sonnet wrapper 已多次驗證 self-rationalize）。
