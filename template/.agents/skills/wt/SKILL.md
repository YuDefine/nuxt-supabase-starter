---
name: wt
description: "Orchestrate a coding task inside a fresh git worktree end-to-end. /wt TASK builds a worktree, dispatches a subagent into it with the task, and reports back — the worktree is preserved with its committed work until /spectra-archive absorbs it (per [[worktree-default]] §5.5). Supports parallel multi-task via /wt A: ... B: .... Use when 使用者說「worktree」「開 wt」「並行做」，或需要在隔離環境跑 coding task 不影響主 working tree。"
license: MIT
metadata:
  author: clade
  version: "3.0"
