...
This generated AI Agent skill runs with `context: fork`. The rules in this section take precedence over the shared `verify` body below.
...
1. **If no change name provided, prompt for selection**
   ...
   Run `spectra list --json` to get available changes. Use the **request_user_input 工具** to let the user select (if this tool is not available, ask as plain text and wait for the user's response).
