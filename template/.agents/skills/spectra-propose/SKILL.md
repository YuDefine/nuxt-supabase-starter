...
11. **Park the change and end the workflow**
    ...
    **Post-park commit reminder**（clade fork addition；not in upstream spectra）
    ...
    - **Option A — 立刻 commit artifacts 到 git（recommended）**：先 `spectra unpark "<change-name>"` 把 artifacts 還原到 disk，然後在 main worktree（**禁止**在 subagent / ephemeral worktree）selective stage 並 commit：

      ```bash
      spectra unpark "<change-name>"
      git add openspec/changes/<change-name>/
      git commit -m "📝 docs(spectra): propose artifacts for <change-name>"
      ```
