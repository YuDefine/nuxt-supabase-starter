...
- 用 `request_user_input` 問 staged 內容代表什麼意圖
...
0. **Dispatch 路徑選擇（三選一選單）**
   ...
   本 skill 的 draft 階段有三條可選路徑。**Step 0 開頭 MUST 用 request_user_input 跳三選一選單**讓使用者選（除非使用者已明確指定路徑，見下方捷徑）：

   - **A. Codex flow（預設 / 推薦，選單第一項）** — Codex GPT-5.5 xhigh draft + 主線 Claude Opus 4.8 xhigh cross-check。draft + cross-check 比擇一穩，wall-clock 最短。
   - **B. Fable pipeline** — Claude Fable 5 High draft → Codex GPT-5.5 xhigh 檢查（只出 findings、不改檔）→ 主線 Claude Opus 4.8 xhigh final check。三模型交叉，wall-clock 較長（多一層背景等待）。
   ...
   **明確指定捷徑（跳過選單，直接走對應選項）**：使用者訊息已明確指定路徑時**不問選單**：
   ...
   - 「用 Fable」「走 Fable pipeline」「Fable 起草」→ **選項 B**
   ...
   ### 選項 B：Fable pipeline（Fable draft → Codex 檢查 → 主線 final check）

   三段序列：Claude Fable 5 High 在背景起草 → Codex GPT-5.5 xhigh 檢查出 findings → 主線 Claude Opus 4.8 xhigh 整合 findings 並完成全套 cross-check。三段皆背景派工 + notification-only watch（per `.claude/rules/agent-routing.codex-watch-protocol.md` § 監看排程 A）。

   #### Phase B-0a：背景 headless Fable draft

   1. **解析 change name + requirement**（同 Phase 0a step 1）。
   2. **Write prompt 檔到 `/tmp/fable-spectra-propose-<change-name>-prompt.md`** — 內容**完全沿用 Phase 0a step 2 的 draft prompt 範本**（Plan-first / Phase Purity / Manual Review Kind Marker / Backend-only 規約 / `docs/FIXTURES.md` sample / 語言遵循 / `spectra park` 完成標準全部照搬）。Fable 與 codex 讀同一份 repo SKILL + rules，只是 runtime 不同。
   3. **背景啟動 headless Claude**（**Bash** tool 加 `run_in_background=true`）：

      ```bash
      cd <consumer-repo-root> && cat /tmp/fable-spectra-propose-<change-name>-prompt.md | claude -p \
      ...
      **NEVER** 加 `--bare`（會載不到 repo 的 SKILL / rules，Fable 無法執行流程）。預設 text 輸出，不加 `--output-format json`（與 codex 一致，主線讀 tail）。
   4. **立刻**簡短回報：「已派 Claude Fable 5 High 在背景 draft `<change-name>`（bash job `<id>`）；完成後派 Codex 檢查，再由主線 Opus final check」。
   5. 啟動 **Watch Protocol**（同 Phase 0a step 5）— **notification-only**：headless `claude -p` 背景 job 同屬「主線直接 Bash 派」路徑，主線 idle 等 `<task-notification>`，只下**一個**安全網 fallback `ScheduleWakeup(1500, "fable spectra-propose <change-name> 安全網檢查 — 預期靠 task-notification 收尾")`。**NEVER** 短輪詢。

   #### Phase B-0b：Fable 完成 → 派 Codex 檢查

   收到 Fable `<task-notification status=completed>` 時**立刻**：

   1. **Read Fable stdout** 摘要：BashOutput 讀完整 stdout，回報 artifacts list / `spectra validate` 結果。
   2. **若 Fable 已 `spectra park <change-name>`：先 `spectra unpark <change-name>`** — park 後 artifacts 只存 `.git/spectra-app/spectra.db` SQLite blob、不在 git tracked file，codex 要 review 必須先 unpark 回檔案系統（同 Phase 0b step 2 + park pitfall）。
   ...
   5. **立刻**回報：「Fable draft 完成，已派 Codex GPT-5.5 xhigh 檢查（bash job `<id>`）；完成後主線 Opus final check」+ 啟動 notification-only watch（同 Phase B-0a step 5）。
   ...
   2. **主線整合 findings + 跑完整 cross-check** — 執行 Phase 0b step 3 ~ 9 全套（`post-propose-check.sh` / `post-propose-manual-review-check.sh` / `design-inject.sh` / 補 Design Review 7 步 / Manual Review Marker Hygiene / `[verify:auto]`→explicit marker / Backend Verification Evidence 搬移 / Open Questions→request_user_input / `spectra analyze` / `spectra validate` / `spectra park`），並把 codex findings 一併納入修補依據。
   3. **主線自己 Edit 修**（**NEVER** 把修補丟回 Fable / codex，太慢、來回成本高）。
   4. 回報使用者：artifacts list + Fable draft 摘要 + **codex findings 摘要** + 主線補了什麼（Design Review 7 步 OK 與否、analyze/validate 結果）+ `/spectra-apply <change-name>` 提示。
   ...
   - **NEVER** 派 draft（codex 或 Fable）後不跑 cross-check / final check（post-propose-check + design-inject + 主線補 Design Review 7 步 + spectra analyze）
   - **NEVER** 把 cross-check / final check 的修補工作丟回 codex / Fable（太慢、來回成本高）— 主線**自己** Edit 修
   - **NEVER** 沉默等使用者來問進度；通知一到自己讀檔 + 續跑後續 phase
   - **NEVER** 派 draft（codex 或 Fable）而 prompt 漏掉 Plan-first 段落 — 必須在動筆前先輸出 `## Plan`（要動哪些檔 / 每檔寫什麼 / phase 切分），主線後續 check 才有對齊基準

   **選項 B 專屬 NEVER**：

   - **NEVER** 給 Fable headless 加 `--bare`（會載不到 repo SKILL / rules，無法執行流程）
...
11. **Park the change and end the workflow**
    ...
    **Post-park commit reminder**（clade fork addition；not in upstream spectra）
    ...
    Park 之後 artifacts 只存在 `.git/spectra-app/spectra.db` SQLite blob（**不在 git tracked file**）。若後續 `/spectra-apply` 的 unpark 步驟在錯誤 cwd 跑（例如 AI Agent `Agent` tool dispatched subagent 的 ephemeral `.claude/worktrees/agent-*/`），unpark 寫的 artifacts 會落在 session-GC 路徑 → SQLite parked 條目同時被 unpark 刪除 → **artifacts 在 git / SQLite / 任何活著的 worktree 都不存在 = 永久遺失**（已在 co-purchase 撞過：99 tasks + 5 specs + proposal 蒸發，僅 design.md 從 git history 復原但缺最後 3 個決策）。完整 root cause 見 `docs/pitfalls/2026-05-22-agent-tool-subagent-worktree-bypass.md`。

    Park 跑完並輸出 Handoff message 之後，**MUST** 用 **request_user_input** 給 user 二擇一（這條問題是「commit-to-git for data-loss safety」，跟前段「do NOT call request_user_input to ask whether to park or apply」是不同議題；本 request_user_input 必跑）：
    ...
    - **Option B — 維持 parked（接受風險）**：保留 SQLite blob 狀態，artifacts 不寫 disk、不上 git。**僅當** user 明確知道下一步 `/spectra-apply` 不會經過 `Agent` tool dispatched subagent（例如直接在 main 跑 `/spectra-apply` 而非 `/wt` Form 3）才安全。
      ...
      Option description 必含風險揭露：`若下一步 /spectra-apply 透過 spawn_agent 工具 dispatch subagent（含 /wt Form 3）→ subagent cwd 是 .claude/worktrees/agent-*/，unpark 寫進去後 session GC = artifacts 永久遺失`。

    User 選 Option A → 主線執行上述 unpark + commit 動作。User 選 Option B → 跳過 commit，**MUST** 額外輸出一行警告：`⚠ Parked artifacts only exist in SQLite blob; if /spectra-apply runs through spawn_agent 工具 dispatch, artifacts may be lost. Consider committing now.`

    若 **request_user_input** tool 不可用 → 以純文字列出 A / B 兩選項 + 上述風險揭露，等 user 回答後執行對應動作。

    **NEVER** 跳過此 request_user_input（理由：本 prompt 不是「要不要 park / apply」的決策問題，是「park 之後 commit-to-git for data-loss safety」的問題；前段禁止的是前者）。
