---
name: review-rules
description: 管理兩層 review 規則清單（clade / project）— 新增、修改、刪除、列出、跨 consumer survey、promote 到共用層。此 skill 只負責規則檔內容管理，不執行 code review。
---
...
| 層 | 路徑 | 治理 | 觸發場景 |
| --- | --- | --- | --- |
| clade | `.codex/agents/references/clade-review-rules.md` | clade LOCKED | 跨 consumer 共用嚴格條目（目前是 Nuxt + Supabase stack baseline） |
| project | `.codex/agents/references/project-review-rules.md` | 該專案自管 | 該專案在地化條目 |

`code-review` agent Step 0 依序 Read 兩份；`vendor/scripts/review-checklist-audit.mjs` 在 commit-time 跑兩份的 grep pattern 做硬 gate。**clade 層必須走 clade 中央倉維護**，不能在 consumer 端編輯（chmod 444 + sync 還原）。
...
- 使用者說「promote 這條 / 升到共用」/「promote this rule to clade」→ Clade 層 flow 的「從 project promote 上來」
...
- cwd 是 `~/offline/clade` → project 那個 option 加備註「（目前在 clade，要切到 consumer 才能寫 project 層）」
- cwd 是 consumer（如 `~/offline/perno`）→ clade 那個 option 加備註「（目前在 consumer，要 `cd ~/offline/clade` 才能寫 clade 層）」
...
對該專案在地條目操作 `.codex/agents/references/project-review-rules.md`（不存在則建立）。
...
2. Read 既有 `clade-review-rules.md`，定位要改的地方（新增 → 找適合分類 section 的末尾；修改 / 刪除 → 對應 `## <規則名稱>` h2）
3. 寫入規則 — 必含五個欄位（同 Project flow 規則格式，**沒有例外**：不能省 grep pattern，audit script 解析這部分做硬 gate）
...
6. **告知散播 SOP**（不自動執行 — destructive op 需 user 確認）：
   ```
   git add plugins/hub-core/agents/references/clade-review-rules.md
   git commit -m "feat(review-rules): add clade rule '<title>'"
   # 然後在 clade session 講「走 /clade-publish 散播」
   ```

### 從 project promote 上來（升級用法）

跟「新增條目」步驟相同 — 把 consumer 端 project 層條目 copy 過來貼進 clade 層。**不要**自動從 consumer 端 cut 走原條目；讓 user 自己決定要不要從 project 層刪除（user 可能想保留 project 層覆蓋）。
...
2. 對每個 consumer 讀 `.codex/agents/references/project-review-rules.md`（不存在則 skip）
3. 比對策略：
...
4. 輸出 **promotion 候選表**：
   ...
   | --- | --- | --- | --- | --- |
   | ... | perno, TDMS, sroi | 3 | clade（重複 ≥ 3） | 規則內容一致 |

5. **不寫任何檔**；交給 user 決策後再走 Clade 層 flow 的「從 project promote 上來」
...
- 兩層規則檔位置固定：`.codex/agents/references/{clade,project}-review-rules.md`
...
- clade 層只能在 clade 改 → propagate；consumer 端是 LOCKED 投影
