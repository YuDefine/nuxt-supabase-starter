...
> Clade 鎖定版本見 `../SKILL.md` Prerequisites 區塊（目前 v3.9.0）。新 consumer 對齊本檔即可，不要從歷史 install-skills.sh copy v2 拆分形態。
...
```bash
# Impeccable Design Skill（pbakaus/impeccable — 單一 skill 含 23 sub-command + pin/unpin/hooks 三個 management command；clade design orchestrator 鎖定 v3.9.0）
