---
name: design
description: UI/UX design orchestrator — coordinates multiple design skills into plans. Use for /design new, /design improve, /design iterate, /design health (full-stack deep health audit → refactor roadmap → propose). NOT for coding UI or single-skill tasks.
---
...
1. **前置**：建 graph（未建先 index）+ 定 scope + 讀既有真相層（`docs/tech-debt.md` / `ROADMAP.md` / `PRODUCT.md` / `DESIGN.md`）
...
5. **Propose funnel**：讓 user 挑最高優先 wave → 跑 **`/spectra-propose`**（brief 帶 findings + file:line + 卓越 delta 驗收）；其餘 wave 登 ROADMAP / tech-debt
