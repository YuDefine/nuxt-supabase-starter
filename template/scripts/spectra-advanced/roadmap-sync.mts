 ...
 *
 * Configuration: reads `spectra-advanced.config.json` (legacy
 * `spectra-ux.config.json` still accepted) at the project root.
 ...
 *
 * Usage:
 *   node scripts/spectra-advanced/roadmap-sync.mts           # full sync (default)
 *   node scripts/spectra-advanced/roadmap-sync.mts --check   # validate, no write
 *   node scripts/spectra-advanced/roadmap-sync.mts --json    # emit report as JSON
 *
...
function findRepoRoot(): string {
  // Prefer spectra-advanced.config.json as the canonical anchor (legacy
  // spectra-ux.config.json still accepted). Fallback to .git for installs
  // without config, final fallback is script dir's grandparent.
  let dir = __dirname
...
function loadConfig(): Config {
  ...
  try {
    ...
    console.error(`roadmap-sync: failed to read ${configPath}: ${err}`)
    return defaults
  }
}
...
function renderClaimsBlock(claims: ClaimView[], enabled: boolean): string {
  ...
  if (!enabled) {
    return [...intro, '_Claims disabled in `spectra-advanced.config.json`._'].join('\n').trimEnd()
  }
...
function scaffoldRoadmap(): string {
  return [
    '# OpenSpec Roadmap',
    '',
    '> Maintained by spectra-advanced. AUTO blocks are regenerated by `spectra:roadmap`;',
